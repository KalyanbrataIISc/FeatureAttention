...
%   - Lives system: participant starts with numLives lives; each ball drop costs
%     one life; block ends early when lives reach 0
...
if ~ismac
    ...
    try
        ...
        warning('gameBreakoutv2:cedrusFailed', 'Could not open Cedrus box: %s. Keyboard will be used instead.', ME.message);
        cedrusAvailable = false;
    end
end
...
if ~ismac
    try
        ...
        warning('gameBreakoutv2:paraportFailed', 'Could not open parallel port COM9: %s. Triggers will not be sent.', ME.message);
        paraport = [];
    end
end
...
bgColor    = [10  12  20  ];    % near-black blue-tinted background
neonBlue   = [0   200 255 ];    % ball color
neonGreen  = [0   255 140 ];    % brick hit flash
cyanMid    = [30  30  60  ];    % paddle body (dark blue-grey)
cyanBorder = [0   180 220 ];    % paddle border (bright cyan)
brickBaseColor  = [255 80  40  ];    % brick fill (vivid orange-red)
brickBorderCol  = [255 180 60  ];    % brick border (gold)
scoreHudColor   = [220 220 255 ];    % HUD text (soft lavender-white)
trialHudColor   = [150 150 200 ];    % trial counter (dimmer)
readyColor      = [0   255 200 ];    % 'Get ready' text (teal)
...
% ---- Paddle geometry ----
paddleWidthPx        = 600;   % total width, including both gratings
paddleHeightPx       = 200;    % taller = more grating area = stronger SSVEP drive
...
% ---- Ball ----
ballRadiusPx           = 12;
ballSpeedPxPerSec      = 360;  % constant for the whole trial - nothing changes it
...
magnusGainPerFrame      = 0.01;    % how strongly spin curves the flight; 0 disables curving
...
% ---- Colors ----
% Grating colors: per-side chromatic pairs — left = yellow/dark-yellow, right = cyan/dark-cyan.
gratingMidColor        = grey;              % grating mid = background grey (SSVEP requirement)
gratingLeftBarHigh     = [255 220   0];     % left grating: bright yellow
gratingLeftBarLow      = [100  80   0];     % left grating: dark yellow
gratingRightBarHigh    = [  0 230 230];     % right grating: bright cyan
gratingRightBarLow     = [  0  80  80];     % right grating: dark cyan

% Game object colors: updated for dark theme
paddleBodyColor    = cyanMid;       % was [70 70 70]
paddleBorderColor  = cyanBorder;    % was black
ballColor          = neonBlue;      % was white
brickColor         = brickBaseColor;% was [220 60 60]
brickBorderColor   = brickBorderCol;% was black

% ---- Background ----
backgroundColor    = bgColor;       % near-black; replaces grey in all FillRect background calls
missTextColor      = [255 60  60 ];   % bright red for 'MISS!' display
...
scoreTextSize        = 24;
scoreTextYPx         = 25;
scoreTextColor       = scoreHudColor;   % soft lavender-white on dark background

% ---- Scoring (v2) ----
% Per-brick score = brickBasePoints
%                 + bounceBonus * paddleBouncesSinceBrick
%                 + timeBonusRate * trialElapsedSec (at moment of brick break)
% This rewards both sustained rallying (bounce streak) and keeping the ball
% alive longer (time component).
brickBasePoints      = 100;   % flat points for breaking any brick
bounceBonus          = 10;    % extra points per paddle bounce since the last brick was broken
timeBonusRate        = 5;     % extra points per second elapsed in the trial when brick breaks
% Cumulative block score and per-trial scores are tracked in blockTotalScore and
% trialScores arrays (initialised in the Block Loop setup, below).

% ---- Lives (v2) ----
% The participant starts with numLives lives. Each ball drop costs one life.
% When lives reach 0, runBlockLoop is set false and the block ends early.
numLives             = 10;    % starting lives per block
missDisplaySec       = 1.5;   % how long 'MISS!' and lives-remaining text is shown
% NOTE: missDisplayFrames is computed in %% Derived values (needs interFrameInterval)

% ---- Block-level timer (v2) ----
% The entire block must finish within blockTimeLimitSec seconds. The countdown
% is checked between trials (not mid-frame), so the current trial always completes
% before time-out is enforced. Remaining time is shown in the HUD (bottom-centre)
% as MM:SS, turning red when under 30 seconds. blockStartTime is captured just
% after the instructions screen.
blockTimeLimitSec    = 360;   % 6 minutes; set to Inf to disable
...
trialScores      = zeros(1, trialNumberPerBlock);   % v2: score earned in each trial
blockTotalScore  = 0;                               % v2: cumulative score across all trials
livesRemaining   = numLives;                        % v2: lives left in the block
blockTimedOut    = false;                           % v2: set true when block clock expires
...
try
    ...
    DrawFormattedText(window, instructionsText, 'center', 'center', scoreHudColor, [], [], [], [], [], instructionsRect);
    Screen('TextSize', window, 28);
    DrawFormattedText(window, 'Press any key or button to begin', 'center', windowRect(4) - 80, readyColor);
    ...
    blockStartTime = GetSecs;   % v2: block countdown starts after participant acknowledges instructions
    ...
    while runBlockLoop && trialNumber <= trialNumberPerBlock
        ...
        % v2 scoring: track paddle bounces and elapsed time since the last brick break
        trialScore              = 0;    % score accumulated in this trial
        paddleBouncesSinceBrick = 0;    % reset when a brick is broken; increments on each paddle bounce
        lastBrickBreakTime      = NaN;  % wall-clock time (experimentStartTime-relative) of trial start;
        ...
        for currentFrame = 1:maxTrialFrames
            ...
            % Force speed in that direction to 0 if the paddle is against the screen edge
            if paddleCenterX <= paddleMinCenterX && paddleVxPxPerFrame < 0
                paddleVxPxPerFrame = 0;
            elseif paddleCenterX >= paddleMaxCenterX && paddleVxPxPerFrame > 0
                paddleVxPxPerFrame = 0;
            end
            ...
            if hitPaddle
                ...
                paddleBouncesSinceBrick = paddleBouncesSinceBrick + 1;   % v2 scoring
            end
            ...
            if brickActive
                ...
                if hitBrick
                    ...
                    trialElapsedSec = currentTime - trialStartTime;       % seconds since trial start
                    brickPoints     = round(brickBasePoints ...
                                    + bounceBonus   * paddleBouncesSinceBrick ...
                    ...
                    % Original deactivation logic (unchanged)
                    brickActive   = false;
                    brickRect     = [NaN NaN NaN NaN];
            ...
            % --- Paddle (unchanged draw call, recolored via paddleBodyColor/paddleBorderColor) ---
            drawPaddle(window, paddleRect, paddleGratingWidthPx, gratingBarWidthPx, ...
                leftBarColorA, leftBarColorB, rightBarColorA, rightBarColorB, ...
                paddleBodyColor, paddleBorderColor, paddleBorderPx);
            ...
            if showScore
                ...
                % Top-left: running cumulative score (block total so far + current trial)
                % Formatted as a whole number using %.0f to avoid scientific notation
                runningScore = blockTotalScore + trialScore;
                DrawFormattedText(window, sprintf('SCORE: %.0f', runningScore), ...
                    fieldLeft + 30, 'center', scoreHudColor, [], [], [], [], [], ...
                    [fieldLeft, windowRect(2), fieldLeft + 300, fieldTop]);

                % Top-right: lives remaining + trial counter
                hudRight = sprintf('LIVES: %d  |  TRIAL %d/%d', livesRemaining, trialNumber, trialNumberPerBlock);
                DrawFormattedText(window, hudRight, 'right', 'center', livesHudColor, [], [], [], [], [], ...
                    [0, windowRect(2), fieldRight - 30, fieldTop]);

                % Top-centre: block countdown timer (MM:SS), turns red under 30 s
                blockElapsed   = GetSecs - blockStartTime;
                ...
                if blockRemaining <= 30
                    timerColor = missTextColor;   % bright red — urgent
                else
                    timerColor = scoreHudColor;   % match score color inside the HUD
                end
                DrawFormattedText(window, sprintf('%02d:%02d', timerMins, timerSecs), ...
            ...
            if ball.y - ball.radius > fieldBottom
                ...
                for missFrame = 1:missDisplayFrames
                    ...
                    DrawFormattedText(window, 'MISS!', 'center', yCenter - 60, missTextColor);
                    Screen('TextSize', window, 36);
                    if livesRemaining > 0
                        livesMsg = sprintf('Lives remaining: %d', livesRemaining);
                    else
                        livesMsg = 'No lives remaining!';
                    end
                    DrawFormattedText(window, livesMsg, 'center', yCenter + 20, livesHudColor);
                    if ~ismac
                        vbl = Screen('Flip', window, vbl + 0.5 * interFrameInterval);
                    else
                        vbl = Screen('Flip', window);
        ...
        trialScores(trialNumber)          = trialScore;                      % v2
        blockTotalScore                   = blockTotalScore + trialScore;    % v2
        ...
        % v2: end block early if no lives remain
        if livesRemaining <= 0
            runBlockLoop = false;
        end

        % v2: end block early if the block time limit has expired
        if GetSecs - blockStartTime >= blockTimeLimitSec
            blockTimedOut = true;
            runBlockLoop  = false;
        end
...
%% Calculate and display block results (v2)
...
% Determine how the block ended
if blockTimedOut
    endReason = 'TIME LIMIT REACHED';
elseif livesRemaining <= 0
    endReason = 'NO LIVES REMAINING';
else
    endReason = 'ALL TRIALS COMPLETED';
end

% Build per-trial summary string
perTrialLines = 'Per-trial scores:\n';
for t = 1:trialNumberPerBlock
    if completedTrials(t)
        perTrialLines = [perTrialLines, sprintf('  Trial %2d: %d bricks  |  %d pts\n', ...
            t, bricksBrokenByTrial(t), trialScores(t))]; %#ok<AGROW>
    end
end
...
performanceText = sprintf([ ...
    ...
    endReason, finalScore, totalBricksBroken, livesUsed, numLives, meanTrialDuration, perTrialLines);
