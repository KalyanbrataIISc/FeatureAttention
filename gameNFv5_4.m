% gameNFv5_3.m - gameNFv5_2.m with fixed-path leaf motion.
%
% (Named with an underscore, not "gameNFv5-3": MATLAB script filenames must
% be valid identifiers, and a hyphen would be parsed as subtraction, making
% the script impossible to invoke by name.)
%
% Identical to gameNFv5_2.m in cue, goal, neurofeedback, timing, triggers and
% CSV output. The only change is that the leaves now follow permanent
% wrapping paths that never overlap, instead of being respawned whenever a
% lifetime expires or two leaves drift together - see the FIXED-PATH LEAF
% MOTION block below. gameNFv7.m is the same change applied to gameNFv6.m.
%
% The two branches off gameNFv4.m, recombined: the cue and the goal are
% gameNFv5.m's (the centre box names a feature and a direction, e.g.
% 'MOVING' / 'UP', and the participant reports the COLOUR of the flock that
% identifies - left arrow = orange = c2, right arrow = blue = c1), while the
% neurofeedback is gameNFv6.m's (black display, black-to-white SSVEP
% flicker, an integrated grayscale leaf fill, a green zone that must be held
% to reveal the flock colours, and no response accepted until it is). See
% the NEUROFEEDBACK PARAMETERS block below for the full NF mechanism.
%
% Why this combination is the tightest of the three. In gameNFv6.m the two
% flocks are visually identical until the reveal, so the participant cannot
% tell which one the coloured cue box is asking about and can only infer
% they are attending correctly from the level climbing. Here the cue names a
% *direction* instead of a colour, and direction is still perfectly visible
% in grayscale - the flocks move and point exactly as before. So the
% participant can identify and attend the cued flock throughout the NF
% phase, and the reveal supplies precisely the one thing they still lack:
% its colour, which is the answer. The neurofeedback gates the answer rather
% than gating the ability to start working, and gameNFv5.m's design already
% assumed exactly that (its nfBaselineDeltaE was 0 for the same reason).
...
% This variant's CSV is gameNFv5.m's columns plus gameNFv6.m's, so it
% carries both sets of additions over gameNFv4.m.
%
% From gameNFv5.m (the task):
%   CueFeature      - 'moving'/'pointing', and
%   CueDirection    - the cued 'up'/'down'/'left'/'right'. Together they are
%                     the literal cue that was displayed. Cue is KEPT and
%                     still valued 'c1'/'c2' (the flock the cue points at) -
%                     it is what fixes the attended SSVEP frequency and the
%                     nf.txt column, and what the offline analysis scripts
%                     group by. CorrectResponse/ParticipantResponse now hold
%                     'blue'/'orange' (or 'missed') rather than a direction.
%
% From gameNFv6.m (the neurofeedback):
...
%                     (NaN if that never happened). This supersedes
%                     gameNFv5.m's ColorRevealTime, which meant the same
%                     thing under the old NF (first frame drawn with
%                     nfDrive > 0). It matters more here than in either
%                     parent: the colour IS the answer, so this is the
%                     moment the trial first became answerable at all, as
%                     well as when the response window opens and what
%                     ReactionTime is measured from.
%   RevealTimeout   - 1 if the trial ended because colours never appeared
%                     within nfRevealTimeoutSec of cue onset. Distinct from
%                     ResponseTimeout, which means the colours DID appear
%                     but no response followed within responseTimeoutSec.
%                     The two are mutually exclusive.
csvHeader = ['TrialNumber,TrialStart,C1PointDir,C1MoveDir,C2PointDir,C2MoveDir,Cue,CueFeature,CueDirection,' ...
    'CueOnsetTime,FirstGreenTime,ColorOnsetTime,CorrectResponse,ParticipantResponse,Accuracy,ReactionTime,' ...
...
% The response is 2-alternative (a colour), so only left/right are read -
% gameNFv6.m's up/down arrows are not needed here.
...
% The cue is not a colour naming a flock - it is a (feature, direction)
% pair, e.g. "MOVING UP", and the participant reports the COLOUR of the
% flock it identifies (left arrow = orange = c2, right arrow = blue = c1).
...
%
% It also means the cue is legible while the leaves are still pure
% grayscale, which is what makes this variant's neurofeedback gate the
% *answer* rather than the participant's ability to start - see the file
% header.
%
...
% both features and both cued flocks/correct colours - occur equally often at
...
% ===================== FIXED-PATH LEAF MOTION (from v7) ====================
% gameNFv5_2.m and every variant before it moved the leaves like an RDK:
...
%
% Here every leaf instead follows a permanent wrapping path, chosen at trial
...
%
% This matters more here than in gameNFv7.m. The cue names a direction, so
% the participant has to FIND the flock moving or pointing that way while
% the leaves are still uniform grey - and a leaf that vanishes and reappears
% elsewhere mid-search is precisely the wrong stimulus for that. Continuous
% paths make the cued flock trackable from cue onset.
%
...
% Cue rectangle (same object throughout the trial: dim grey pre-cue, then
% dark green from cue onset on with the two-line cue - feature above
% direction, e.g. 'MOVING' / 'UP' - written inside it in white, switching to
% the 'Correct'/'Incorrect' feedback text in the same box once a response is
% given).
% The box is deliberately NOT filled with the cued flock's colour: that
% colour is the answer here, so colouring the box would give it away. It is
% a fixed dark green for every trial instead - neutral with respect to both
% flock colours, and a much smaller luminance step from the pre-cue box than
% a white box would be, which matters for a fixation-centred stimulus in an
% SSVEP recording. It is also bigger than gameNFv6.m's, to fit two lines.
cueRectWidthPx       = 280;
cueRectHeightPx      = 120;
cueRectCornerRadiusPx = 20;
cueTextSize          = 32;
colorCueText         = white;
% The pre-cue box is a dim grey rather than gameNFv5.m's black, because this
% variant's background is black and a black box on it would be invisible.
% Kept dim deliberately - it sits at fixation for the whole pre-cue period,
% and a bright box there is a large luminance step into cue onset.
colorCueRectPreCue   = [64 64 64];
colorCueRectPostCue  = [0 100 0];

% Response mapping - fixed for the whole experiment, and spelled out on the
% instructions screen. Left arrow = orange = flock c2; right arrow = blue =
% flock c1.
responseLabelC1 = 'blue';    % colorC1 (deep sky blue), reported with the RIGHT ARROW
responseLabelC2 = 'orange';  % colorC2 (dark orange), reported with the LEFT ARROW

% Feedback text colours. Lighter than plain green/red because the feedback is
% drawn inside the dark green cue box - plain green would be near-invisible
% on it and plain red is low-contrast against it.
...
% below), not a per-flock quantity.
%
% In gameNFv6.m that identical rendering is a real cost - its cue is a
% colour, so a participant who cannot see the colours cannot tell which
% flock the cue meant. Here it costs nothing: the cue is a feature and a
% direction, and both flocks keep pointing and moving visibly throughout the
% grayscale phase, so the cued flock is identifiable from frame one. The
% only thing the reveal adds is its colour - which is exactly the thing
% being asked for. See the file header.
%
...
% Level units per second at |NF| = 1. Pure black to pure white then takes
...
% Choose the leaf field, then prove the whole block can actually run on it
% BEFORE the first trial. Every direction combination is placed (and
...
if ~fieldInfo.feasible
    sca;
    error('gameNFv5_3:noWorkableField', ...
...
for comboCheckIdx = 1:size(directionCombos, 1)
    ...
    if ~checkInfo.feasible
        sca;
        error('gameNFv5_3:unplaceableLeaves', ...
    ...
    if verifyLeafPathsAtSetup
        ...
        if ~checkReport.collisionFree
            sca;
            error('gameNFv5_3:leafPathsCollide', ...
...
try
    ...
    % The demo leaves illustrate the colour -> arrow-key mapping, so the orange
    % (c2) example sits on the left and the blue (c1) one on the right,
    % matching the arrow keys they are reported with.
    ...
    instructionsText = [ ...
        ...
        'moving upward. Find that group and keep your attention on it.\n\n' ...
        'The leaves have no color at first. Concentrating on the cued group makes\n' ...
        'them brighten, from black towards white. Once they are nearly white they\n' ...
        'turn green: hold them green and their true colors appear.\n\n' ...
        'Then report the color of the cued group with the arrow keys shown below.\n' ...
        'Keys pressed before the colors appear do nothing. The box will then\n' ...
        'show whether you were Correct or Incorrect.'];
    ...
    DrawFormattedText(window, 'ORANGE\npress the\nLEFT ARROW', 'center', 'center', white, [], [], [], [], [], labelC2Rect);
    DrawFormattedText(window, 'BLUE\npress the\nRIGHT ARROW', 'center', 'center', white, [], [], [], [], [], labelC1Rect);

    Screen('TextSize', window, 28);
    DrawFormattedText(window, 'Press any key to begin', 'center', windowRect(4) - 80, white);
    ...
    while runBlockLoop && trialNumber <= trialNumberPerBlock
        ...
        % cue = which flock this trial's (feature, direction) pair points at.
        % It is still 'c1'/'c2' - it is what fixes the attended SSVEP
        % frequency (and so nfIndex) exactly as in gameNFv4.m/gameNFv6.m -
        % but the participant never sees it directly: they see the feature
        % and the direction, and report the colour of the flock those
        % identify.
        ...
        % Fixed wrapping paths instead of respawning dots. Re-placed each
        ...
        % setup cost (extra per-loop first-frame bookkeeping) right at the
        % visually critical pre-cue/post-cue
        ...
        for currentFrame = 1:maxFrames
            ...
            Screen('FillRect', window, backgroundColor);
            if isPreCue
                % drawLeavesWrapped, not drawLeaves - a leaf straddling an
                % edge has to be painted on both sides, or it would blink out
                % at one edge and reappear at the other, which is the very
                % behaviour the fixed paths exist to remove.
                ...
                % Fixed neutral box - it must not carry the cued flock's
                % colour, since that colour is the answer.
            ...
            if currentFrame == colorOnsetFrame
                ...
                % time is), but getColorResponse's keyboard branch does
                % GetSecs - responseOnsetTime and so needs the raw absolute
                % GetSecs value. Taken from one GetSecs call so the two
                % can't disagree.
            ...
            elseif colorsRevealed
                % Responses are only read once the colours are out. Here that
                % is not merely a fairness rule as it is in gameNFv6.m - the
                % colour IS the answer, so before this point there is
                % literally nothing to report and any press is a blind guess.
                % reactionTime is therefore measured from colour onset, not
                % cue onset.
                %
                % 2-alternative colour response (left = orange = c2, right =
                % blue = c1) rather than gameNFv6.m's 4-direction one.
                [validResponse, participantResponse, reactionTime] = getColorResponse( ...
                    true, [], leftKey, rightKey, responseLabelC2, responseLabelC1, colorOnsetAbsTime);
        ...
        fid = fopen(csvFile, 'a');
        fprintf(fid, '%d,%.6f,%s,%s,%s,%s,%s,%s,%s,%.6f,%.6f,%.6f,%s,%s,%d,%.6f,%d,%d,%.6f,%d\n', ...
...
performanceText = sprintf(['TESTING COMPLETED!\n\n' ...
    'ACCURACY: %.2f%%\n\n' ...
    'Press ESCAPE to continue'], 100 * meanAccuracy);
...
% Values are defined in functions/bci_send_triggers.m and sent by UDP in
% both testing and experiment modes.
% reset      -> 0
% trialstart -> 20
% cueonset   -> 45
