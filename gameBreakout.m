...
% flickering at 17 Hz, its right third a grating flickering at 20 Hz, and the
...
% what moves the paddle: 17 Hz dominance drives it left, 20 Hz dominance
...
traceCsvHeader = ['TrialNumber,FrameNumber,SampleTime,NF17,NF20,NFSigned,NFReadOk,' ...
...
% that GPU load cannot silently detune 17/20 Hz, and this file makes the drop
...
trialNumberPerBlock = 20;     % one trial = one ball, from spawn to fall
...
% Prefer whole seconds here: 17 Hz and 20 Hz both complete an exact whole
...
gratingLeftFreqHz    = 17;    % left grating flicker (drives the paddle LEFT)
gratingRightFreqHz   = 20;    % right grating flicker (drives the paddle RIGHT)
...
% nf.txt is a 3-element binary double vector [SMI_17gt20, SMI_20gt17,
...
% positive when 17 Hz SSVEP power exceeds 20 Hz, index 2 when 20 Hz exceeds
% 17 Hz. The signed drive is nf20 - nf17: positive moves the paddle right,
...
nfIndex17                 = 1;    % nf.txt column: positive when 17 Hz > 20 Hz
nfIndex20                 = 2;    % nf.txt column: positive when 20 Hz > 17 Hz
paddleNfGainPxPerSec      = 600;  % paddle speed per unit of (nf20 - nf17)
paddleMaxSpeedPxPerSec    = 600;  % clamp on paddle speed
paddleNfDeadzone          = 0.02; % |nf20 - nf17| below this pins the paddle still
...
try
    ...
    while runBlockLoop && trialNumber <= trialNumberPerBlock
        ...
        for readyFrame = 1:preSpawnFrames
            [newNf17, newNf20, nfReadOk] = readNfPair(nfFilePath, nfIndex17, nfIndex20);
            if nfReadOk
                nf17 = newNf17;
                nf20 = newNf20;
            end

            paddleVxPxPerFrame = computePaddleVelocityFromNf(nf17, nf20, ...
        ...
        for currentFrame = 1:maxTrialFrames
            ...
            % letting a frame-counter clock silently detune 17/20 Hz for the
