...
%
% Cue, goal, neurofeedback mechanism, leaf motion, timing, triggers and CSV
% output are all identical to gameNFv8.m. The only change is WHERE the NF
% numbers come from: nfSourceType (in the NEUROFEEDBACK block below)
% selects either
%
%   'file' - nf.txt, exactly as v8 and every earlier variant do, and
%   'tcp'  - a TCP connection to nf_tcp_server.py, which pushes the very
%            same three doubles over the network instead.
%
% 'tcp' is what lets this script run on a DIFFERENT COMPUTER from the one
% producing the NF: the acquisition machine runs nf_tcp_server.py, this
% machine sets nfSourceType = 'tcp' and nfTcpHost to the acquisition
% machine's address, and no shared drive is needed between them. Everything
% downstream of the read - clipping, the integrator, the green zone, the
% reveal - is unchanged, so a 'file' run and a 'tcp' run of this script are
% the same experiment with the same data files.
%
% Inherited from gameNFv8.m: gameNFv6.m with fixed-path leaf motion.
%
...
%
% v9: the two sources below deliver identical values - nf_tcp_server.py
% pushes the same 24-byte [SMI_19gt23, SMI_23gt19, sampleCount] record
...
nfLevelRatePerUnitNf = 3.000;  % level units per second at |NF| = 1, i.e. 1.0 -> pure black to pure white in 1s of sustained NF = 1
...
%% Open the neurofeedback source
% Deliberately before the PsychToolbox window opens: if the NF source is
% unavailable (server not started, wrong address, firewall) this errors out
% at the console with a diagnosis and nothing to clean up, rather than
% behind a full-screen window. For 'tcp' it also waits for the first real
% record, so reaching the instructions screen already proves NF is flowing.
nfState = openNfSource(nfSourceType, nfFilePath, nfTcpHost, nfTcpPort, nfTcpTimeoutSec);
...
nfStreamStallWarnFrames = max(1, round(nfStreamStallWarnSec / interFrameInterval));
...
runBlockLoop = true;
trialNumber = 1;
nfStreamStallWarned = false; % v9: the stall warning below fires at most once per block
...
try
    ...
    instructionsText = [ ...
        ...
        'Only then can you answer, with the arrow keys - keys pressed\n' ...
    ...
    while runBlockLoop && trialNumber <= trialNumberPerBlock
        ...
        for currentFrame = 1:maxFrames
            ...
            % NF stream starts at trial start (frame 1) and is re-read
            % fresh every displayed frame, regardless of trial phase - only
            % its visual effect (below) is gated to post-cue. A failed read
            % is not a real sample, so nfCurrentValueRaw is simply left at
            % whatever it already was and the trial proceeds to the next
            % flip on that stale value, rather than snapping to a spurious
            % 0/neutral reading. What counts as a failed read differs by
            % source but is handled identically here: on 'file' it is the
            % I/O race of catching nf.txt mid-rewrite (see readNFValue.m),
            % on 'tcp' it is simply that no new record has arrived since the
            % last frame - normal, and true for ~5 of every 6 frames, since
            % the server pushes at ~10 Hz and this loop runs at ~60 Hz.
            [newNfValueRaw, nfReadOk, nfState] = readNfSample(nfState, nfIndex);
            if nfReadOk
                nfCurrentValueRaw = newNfValueRaw;
            end

            % v9: a 'tcp' source that dies mid-block (server stopped, Wi-Fi
            % dropped) looks exactly like a very quiet signal - the level
            % just stops moving - so say so at the console once, rather than
            % letting a whole block run on a frozen value unnoticed. The
            % block is NOT aborted: the trial is already on screen and the
            % participant is mid-task, so the call on whether the data is
            % usable is the experimenter's to make between blocks.
            if ~nfStreamStallWarned && nfState.staleReads >= nfStreamStallWarnFrames
                fprintf('*** NF WARNING: no fresh NF sample for %.1f s (trial %d, frame %d). Source: %s. ***\n', ...
                    nfState.staleReads * interFrameInterval, trialNumber, currentFrame, nfSourceType);
                nfStreamStallWarned = true;
            end
            ...
            if currentFrame == colorOnsetFrame
                ...
                % time is), but getDirectionResponse's keyboard branch does
                ...
                % their keyboard ReactionTime come out as roughly
    ...
    bci_send_triggers('trialstop');
    nfState = closeNfSource(nfState); %#ok<NASGU>
    cleanupExperiment(testing, eyeTrackingStopped, participantInfo, blockInfo, []);
    rethrow(ME);
end

% v9: the NF connection is closed as soon as the block loop is over, before
% the results screen, so the socket is not left open while the experimenter
% reads the numbers.
nfState = closeNfSource(nfState);
