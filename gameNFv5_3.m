...
% identifies - left button = orange = c2, right = blue = c1), while the
...
% flock it identifies (left button = orange = c2, right = blue = c1).
...
% instructions screen. Cedrus Left (button 3) / left arrow = orange = flock
% c2; Cedrus Right (button 5) / right arrow = blue = flock c1.
...
try
    ...
    % The demo leaves illustrate the colour -> button mapping, so the orange
    % (c2) example sits on the left and the blue (c1) one on the right,
    % matching the buttons they are reported with.
    ...
    instructionsText = [ ...
        ...
        'Then report the color of the cued group with the buttons shown below.\n' ...
        'Buttons pressed before the colors appear do nothing. The box will then\n' ...
    ...
    DrawFormattedText(window, 'ORANGE\npress the\nLEFT button', 'center', 'center', white, [], [], [], [], [], labelC2Rect);
    DrawFormattedText(window, 'BLUE\npress the\nRIGHT button', 'center', 'center', white, [], [], [], [], [], labelC1Rect);
    ...
    while runBlockLoop && trialNumber <= trialNumberPerBlock
        ...
        for currentFrame = 1:maxFrames
            ...
            if currentFrame == colorOnsetFrame
                ...
                % time is), but getColorResponse's testing branch does
            ...
            elseif colorsRevealed
                ...
                if ~testing
                    [validResponse, participantResponse, reactionTime] = getColorResponse( ...
                        false, cedrus, leftKey, rightKey, responseLabelC2, responseLabelC1, colorOnsetAbsTime);
                else
                    [validResponse, participantResponse, reactionTime] = getColorResponse( ...
