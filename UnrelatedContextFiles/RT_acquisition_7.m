function [cut_off_init] = RT_acquisition_7(varargin)
...
savepath = filepath; %#ok
mkdir(filepath(1:end-1));

% Feedback
feedback_file_name='X:\Kalyanbrata\nf.txt';
...
pnqL1 = [28 30 32 36 38];    %#ok    % Right Electrodes
pnqL2 = [15 17 5 7 9];  %#ok         % Left Electrodes
...
while(1)
    ...
    data_size_all = [data_size_all,size(data,2)]; %#ok
...
%     ring_buffer=resample(double(ring_buffer'),100,128)';
    ...
    total_errors = [total_errors errors]; %#ok
    errors = 0;
...
%         ring_buffer=resample(double(ring_buffer'),100,128)';
    ...
    if ~isempty(find(trig_values==triggers.trial_stop, 1)) && trial_on
        ...
        timec = char(datetime('now','Format','yyyy-MM-dd-HH-mm-ss')); %#ok
...
%             buff_filt = buff_filt - repmat(mean(buff_filt,2),[1,size(buff_filt,2)]);
             ...
             % Z-score
        if z_score
            buff_filt = zscore(buff_filt')'; %#ok
        end

        for i=1:1 % change 4 to 2 for number of chunks
            buff_filt_epochs(:,:,i)=buff_filt(:,128*(i-1)+1:128*i); %#ok
        end
...
%         end
         ...
         for xs = 1:14
            ...
            SSVEP_power(1) = All_power1(10)./mean([All_power1(9) All_power1(11)]); % 14Hz re
            SSVEP_power(2) = All_power2(14)./mean([All_power2(13) All_power2(15)]); % 18Hz le
            
            SSVEP_power_all(xs,:,cnt) = SSVEP_power; %#ok
            SSVEP_power=[];
            
            Alpha_power(1) = mean(All_power1(4:8)/mean(All_power1)); % 8-12 Hz
            Alpha_power(2) = mean(All_power2(4:8)/mean(All_power2)); % 8-12 Hz
            
            Alpha_power_all(xs,:,cnt) = Alpha_power; %#ok
            Alpha_power=[];FB_out=[]; %#ok

            SS_power_allR(xs,:,cnt) = All_power1; %#ok %SS_power;
            SS_power_allL(xs,:,cnt) = All_power2; %#ok %SS_power;        
         end
...
%          FB_out(2) = cdf(FB_dist(2),avg_SSVEP(2));
        ...
        AMI_all(:,cnt) = avg_AMI; %#ok
        SMI_all(:,cnt) = avg_SMI; %#ok
%         
%         FB_out=(FB_out);
%         if cnt>5
...
%         disp(fb_out_send);
        ...
        fb_out_send_all(:,cnt) = [fb_out_send fb_out_sends]; %#ok
        ...
        ring_buffer_all(:,:,cnt) = ring_buffer; %#ok
        cut_off_allL(:,:,cnt) = squeeze((SS_power_allL(:,:,cnt))); %#ok
        cut_off_allR(:,:,cnt) = squeeze((SS_power_allR(:,:,cnt))); %#ok

% %         change_val_all(:,cnt) = change_val;
        FB_out_all(:,cnt) = FB_out_avg; %#ok
