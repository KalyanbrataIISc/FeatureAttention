...
For the Unity tablet + Biosemi USB experiment, see [Biosemi USB setup](Biosemi-USB-setup.html). The P-system bridge is `run_biosemi_usb_bridge.m`; `Start-BiosemiUsb.ps1` maintains the ADB cable tunnel. `RT_files/RT_acquisition_CCA.m` now treats both trigger 20 and trigger 21 as trial starts. MATLAB R2015 runs the bridge as a function file, with no functions inside an experiment script.
