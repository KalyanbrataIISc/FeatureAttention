...
binary format and write cadence RT_acquisition_8.m uses in the real
experiment room, so gameNF.m/gameNFv2.m can be exercised on mac without real
EEG hardware.

nf.txt format (matches RT_acquisition_8.m): 3 little-endian doubles
    [SMI_17gt20, SMI_20gt17, sampleCount]
gameNF.m/gameNFv2.m read SMI_17gt20 / SMI_20gt17 (1-based indices 1/2), so
...
acquisition side, so the SMI pair moved down to indices 1/2.)
    ...
    try:
        while True:
            ...
            with open(args.path, 'wb') as f:
                f.write(struct.pack('<3d', smi1, smi2, float(sample_count)))
